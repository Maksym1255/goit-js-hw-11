...
`).join("");t.insertAdjacentHTML("beforeend",s),new P(".gallery a").refresh()}const h=document.getElementById("search-form"),x=document.getElementById("search-input"),L=document.getElementById("loader"),g=document.querySelector(".btn-load"),l=document.getElementById("gallery");let i="",c=1,m=1;const M=12;h.addEventListener("submit",async o=>{o.preventDefault(),b(),i=x.value.trim(),c=1;const t=await p(i,c);if(l.innerHTML="",h.reset(),i===""){l.innerHTML="",f.warning({title:"Warning!",message:"Please enter a search query",position:"center"}),d(),u();return}if(t.totalHits===0){l.innerHTML="",f.info({title:"No results",message:"Sorry, there are no images matching your search query. Please try again!",position:"center"}),d(),u();return}try{y(t.hits),d(),E()}catch(s){console.error(s)}});g.addEventListener("click",async()=>{c++,u(),b();try{const o=await p(i,c);y(o.hits),q()}catch{console.log("error")}d(),E()});function b(){L.classList.add("loader")}function d(){L.classList.remove("loader")}async function E(){const o=await p(i),t=Number(o.totalHits);m=Math.ceil(t/M),c>=m?(u(),m&&f.info({title:"The End!",message:"End of collection!",position:"topCenter"})):$()}function $(){g.classList.remove("hidden")}function u(){g.classList.add("hidden")}function q(){const t=l.children[0].getBoundingClientRect().height;scrollBy({top:t*2,behavior:"smooth"})}
